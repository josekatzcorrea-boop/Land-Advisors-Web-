...
${hasHreflang(page.path) ? '  <meta property="og:locale:alternate" content="en_US">\n' : ""}
  ...
  <link rel="stylesheet" href="${prefix}styles-i18n.css?v=20260907">
...
/** URLs placeholder: lead-gate.js intercepta y exige formulario antes de WA/calendario */
const DEFAULT_WA_HREF = "#lead-gate";
const DEFAULT_CAL_HREF = "#lead-gate";
...
function dualCtaPair(opts = {}) {
  ...
  return `<div class="${pairClass}">
          <a href="${esc(waHref)}" class="btn btn-cta-wa" data-site-wa data-track="cta_whatsapp">${CTA_WA_ICON}<span>${esc(waLabel)}</span></a>
          <a href="${esc(calHref)}" class="btn btn-primary btn-glow btn-cta-agenda" data-site-calendar data-track="cta_calendar">${CTA_CAL_ICON}<span>${esc(calLabel)}</span></a>
        </div>`;
}
...
function campaignCtaMarkup(campaign, variant = "float") {
  ...
  if (campaign.leadForm) {
    return `<a href="#campaign-lead" class="campaign-cta campaign-cta--wa${sizeClass}" data-track="cta_lead_form">${waIcon}<span class="campaign-cta__label">${waLabel}</span></a>
          <a href="#campaign-lead" class="campaign-cta campaign-cta--cal${sizeClass}" data-track="cta_lead_form">${calIcon}<span class="campaign-cta__label">${calLabel}</span></a>`;
  }

  // Intent y promo sin form: lead-gate (formulario antes de WA/calendario)
  return `<a href="${DEFAULT_WA_HREF}" class="campaign-cta campaign-cta--wa${sizeClass}" data-campaign-wa data-track="cta_whatsapp">${waIcon}<span class="campaign-cta__label">${waLabel}</span></a>
          <a href="${DEFAULT_CAL_HREF}" class="campaign-cta campaign-cta--cal${sizeClass}" data-campaign-calendar data-track="cta_calendar">${calIcon}<span class="campaign-cta__label">${calLabel}</span></a>`;
}
