# Genera brochure/Land-Advisors-Digital.pdf (formato vertical, listo para WhatsApp/correo)
$brochureDir = Split-Path $MyInvocation.MyCommand.Path -Parent
$root = Split-Path $brochureDir -Parent
$outPdf = Join-Path $brochureDir "Land-Advisors-Digital.pdf"
$port = 8765
$url = "http://127.0.0.1:$port/brochure/index.html"
...
if (-not (Test-Path $outPdf)) {
  Write-Error "No se genero el PDF. Abre brochure/index.html y usa Guardar PDF."
  exit 1
}

$mb = [math]::Round((Get-Item $outPdf).Length / 1MB, 2)
Write-Host "PDF listo: $outPdf ($mb MB)"
Write-Host "Formato vertical · 9 pantallas · ideal para WhatsApp y correo"
