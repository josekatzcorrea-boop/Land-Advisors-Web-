/**
 * PDF móvil: vista screen 430px, altura compacta sin huecos 100dvh.
 */
...
const outPdf = join(__dirname, "Land-Advisors-Brochure-movil.pdf");
const baseUrl =
...
try {
  ...
  await cdp.send("Runtime.evaluate", {
    expression: `
      ...
      document.querySelector('.brochure-toolbar')?.remove();
    `,
  });
  await sleep(600);
  ...
  const { result: heightResult } = await cdp.send("Runtime.evaluate", {
    expression: `(function(){
      const deck = document.querySelector('.brochure-pages');
