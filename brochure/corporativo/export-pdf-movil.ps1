# PDF con vista móvil (430px, CSS screen) — fiel al HTML, no A4
...
$outPdf = Join-Path $corpDir "Land-Advisors-Brochure-movil.pdf"
$port = 8765
$url = "http://127.0.0.1:$port/brochure/corporativo/index.html"
$mjs = Join-Path $corpDir "export-pdf-movil.mjs"
...
if (-not $node) {
  Write-Error "Node.js no está instalado. Necesario para exportar PDF móvil."
  exit 1
}

$env:BROCHURE_URL = $url
$env:PDF_SCALE = "1.5"

Push-Location $corpDir
try {
...
Write-Host "Vista pantalla 430px · compartir por WhatsApp/correo"
...
$updated = [regex]::Replace(
  $html,
  'href="Land-Advisors-Brochure-movil\.pdf(\?v=[^"]*)?"',
  "href=`"Land-Advisors-Brochure-movil.pdf?v=$pdfVersion`""
)
if ($updated -ne $html) {
